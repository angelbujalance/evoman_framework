...
ENEMY = 2  # Specify the enemy number
...
def read_results(experiment_dir, num_runs):
    ...
    for i in range(num_runs):
        results_path = os.path.join(experiment_dir, f'DEAP_runE2{i}', 'logbook.csv')
        try:
    ...
    best_run_idx, best_generation_idx = np.unravel_index(np.argmax(all_best_fitness), all_best_fitness.shape)

    print(f"Highest Best Fitness: {max_fitness_value}, found in run {best_run_idx}, generation {best_generation_idx}")
    
    return best_run_idx, best_generation_idx, max_fitness_value


def get_best_individual_weights(experiment_dir, best_run_idx):
    best_weights_path = os.path.join(experiment_dir, f'DEAP_runE2{best_run_idx}', 'best.txt')
...
def plot_gain_boxplot(gains_approach_1, gains_approach_2, enemy_number):
    ...
    # Plot the gains
    sns.boxplot(data=data, ax=ax, palette=["#3498db", "#e74c3c"])  # Blue for Approach 1, Red for Approach 2
...
if __name__ == '__main__':
    ...
    best_run_idx, best_generation_idx, max_fitness_value = read_results(experiment_dir, num_runs)
    ...
    gains_approach_1 = np.array([])  # Replace with actual gains for Approach 1

    if best_weights is not None:
        # Initialize the environment
        env = init_env(experiment_dir)
