...
files = [f'NEAT_experiment/enemy_1/NEAT_run{run}/results.txt' for run in range(10)]
...
plt.plot(mean_fitness_means.index, mean_fitness_values, label='Mean Fitness', color='blue', linewidth=2)
plt.fill_between(mean_fitness_means.index, mean_ci_lower, mean_ci_upper, color='blue', alpha=0.2)

# Plot the best fitness line with confidence interval
plt.plot(best_fitness_means.index, best_fitness_values, label='Best Fitness', color='green', linewidth=2)
plt.fill_between(best_fitness_means.index, best_ci_lower, best_ci_upper, color='green', alpha=0.2)
...
plt.savefig("mean_best_fitness_evolution_with_CI.jpg", dpi=300, bbox_inches='tight')
